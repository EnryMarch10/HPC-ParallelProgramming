 ...
 */
int cat_map_rectime(int n, const int chunk_size)
{
    assert(n > 0);
    const int it_size = n * n;
    int *it = (int *) malloc(it_size * sizeof(int));

#pragma omp parallel for schedule(dynamic, chunk_size) collapse(2) default(none) shared(n, it, chunk_size)
    for (int y = 0; y < n; y++) {
        for (int x = 0; x < n; x++) {
            int x_cur = x, y_cur = y;
            int i;
    ...
    int index = 0;
#pragma omp parallel num_threads(max_threads) default(none) shared(new_it, it, it_size, chunk_size, index)
    {
...
#pragma omp critical
            ...
            for (int i = my_index; i < my_index + chunk_size && i < it_size; i++) {
                new_it[my_id] = lcm(new_it[my_id], it[i]);
            }
        } while (my_index < it_size);
    }
    free(it);
    int min_it = 1;
...
int main(int argc, char* argv[])
{
    ...
    int chunk_size = 1;
    if (argc < 2 || argc > 3) {
        fprintf(stderr, "Usage: %s image_size [chunk_size]\n", argv[0]);
        return EXIT_FAILURE;
    }
    if (argc == 3) {
    ...
    k = cat_map_rectime(n, chunk_size);
