...
__global__ void kernel_skyline(const float *const P, int *const s, const int N, const int D, int *const r)
{
    ...
    if (g_index < N) {
        ...
        for (int i = 1; i < N; i++) {
            ...
            if (dominates(&(P[i * D]), &(P[g_index * D]), D)) {
                s[g_index] = 0;
            }
        }
        if (!s[g_index]) {
...
int skyline(const points_t *points, int *s)
{
    ...
    kernel_skyline<<<N_BLOCKS, BLKDIM>>>(d_P, d_s, N, D, d_r);
    cudaCheckError();
