...
echo "INPUT: $INPUT"
echo "OUTPUT: ${OUTPUT}-P$((CORES - 1))-R{1-${N_REPS}}.out"
echo -n -e "$((CORES - 1))\t"

for rep in `seq $N_REPS`; do
  if [ -z "$SCHEDULE" ]; then
    RESULT="$( OMP_NUM_THREADS=$((CORES - 1)) "$PROG" < "$INPUT" 2>&1 > "${OUTPUT}-P$((CORES - 1))-R${rep}.out" )"
  else
    RESULT="$( OMP_SCHEDULE="$SCHEDULE" OMP_NUM_THREADS=$((CORES - 1)) "$PROG" < "$INPUT" 2>&1 > "${OUTPUT}-P$((CORES - 1))-R${rep}.out" )"
  fi
